...
function runClaude({ system, prompt }, timeoutMs, cb) {
  ...
  //
  // Плата за такое решение: вместе с авторизацией подтягиваются плагины,
  // навыки и хук старта сессии владельца. Хук подмешивает в промпт текст про
  // суперспособности — для генерации карточки это шум. Если однажды качество
  // текста просядет без видимой причины, проверять надо здесь.
