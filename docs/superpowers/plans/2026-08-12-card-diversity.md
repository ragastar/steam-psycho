...
**Step 3: Что показал замер (2026-08-12) и три поправки к плану**

Средние по чертам подтвердились: упорство 86.4, мастерство 66.9, ветеранство 65.7,
исследование 56.0, социальность 47.3, барахольство 33.8 (n=29).

Три вещи, которые замер сломал:

1. **Линейная сила профиля насыщается ровно так же, как черты.** На 29 строках
   она дала 7 различимых значений, половина выборки — 0.71: часы упираются в
   потолок 5000 почти у всех, возраст аккаунта — тоже. Считать надо по логарифму
   с потолками 20000 часов и 2000 игр.
2. **В таблице разборов не 29 человек, а 8.** Один человек переразбирался семь
   раз, и в перцентиле он весил бы вседьмеро. Выборку надо схлопывать по
   Steam ID, оставляя последний разбор каждого.
3. **Пороги из кода нельзя калибровать по этим восьми.** Это владелец и его
   друзья, у всех 4000–37000 часов и 12+ лет аккаунту. Калибровка по ним сделала
   бы легендаркой только 37 тысяч часов. Пороги ставятся по внешним ориентирам
   (новичок, обычный игрок, активный, задрот, хардкор), а перцентиль по своей
   выборке включится сам, когда наберётся полсотни РАЗНЫХ людей.

Типичные доли тем (4 профиля): огонь 40, лёд 21, природа 26, железо 4, гроза 2,
кристалл 1, кровь 0.5, пустота 0.4, тень 0, тайна 0.
