...
function openOrderFor(accountId: number, steamId64: string, providerName: string): Order | null {
  ...
  // Ключ совпал с закрытым заказом. Соль делает попытку разовой: защита от
  // гонки на ней уже не держится, но гонка здесь означала бы всего лишь второй
  // брошенный заказ, а не вторую оплату — деньги берёт касса, а на кассу
  // человек уходит по одному адресу.
