...
/**
 * Как выглядит типичный прошедший разбор: середина и разброс по каждой черте
 * (замер 2026-08-12 по восьми РАЗНЫМ людям — повторные разборы схлопнуты).
 *
 * Разброс здесь не для красоты. Мерить отклонение в долях от середины нельзя:
 * у барахольства середина низкая (39.8), и любое отличие по нему выглядит
 * огромным — на живом прогоне оно побеждало у пятерых из восьми, и почти все
 * духи оказались из одного пула (три насекомых, два морских жёлудя). Делить
 * надо на собственный разброс черты: тогда «непохож» значит «непохож на фоне
 * того, как эта черта вообще гуляет у людей».
 */
const TRAIT_BASELINE: Record<keyof CardStats, { mean: number; spread: number }> = {
  dedication: { mean: 83.8, spread: 6.7 },
  mastery: { mean: 62.9, spread: 13.8 },
  exploration: { mean: 60.1, spread: 21.0 },
  hoarding: { mean: 39.8, spread: 21.7 },
  social: { mean: 53.4, spread: 14.6 },
  veteran: { mean: 70.1, spread: 18.4 },
};
 ...
 *
 * Отклонение считается в долях собственного разброса черты. Упорство у всех
 * держится в узком коридоре (разброс 6.7), поэтому отличие на десять очков по
 * нему — событие; барахольство гуляет от 17 до 81 (разброс 21.7), и те же
 * десять очков по нему не значат ничего.
 *
 * Берётся модуль: провал по черте говорит о человеке столько же, сколько
 * всплеск.
 */
export function mostDistinctiveTrait(stats: CardStats): keyof CardStats {
  ...
  for (const key of Object.keys(TRAIT_BASELINE) as (keyof CardStats)[]) {
    const { mean, spread } = TRAIT_BASELINE[key];
    const deviation = Math.abs((stats[key] - mean) / spread);
